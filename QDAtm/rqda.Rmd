...
```{r}
# Export the subject ID and open-ended question variables into a csv file from your SAS,Stata, SPSS, etc dataset 
# load this into an R data frame named q64
...
summaryCodings()
#### Plots
You can create plots to describe your codings from the Code Categories panel.  
...
```{r eval=FALSE}
# You will need to open project first
# Get codes and file names
myCodes<-RQDAQuery("select  * from source inner join coding where source.id = coding.fid and coding.status = 1")
# If you have code categories or file categories?
# Then need to reshape into file id (or case)level
```
